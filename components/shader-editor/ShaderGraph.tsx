...
export function ShaderGraph() {
  ...
  const onNodesChange = useCallback((changes) => setNodes(changes), [setNodes]);
  const onEdgesChange = useCallback((changes) => setEdges(changes), [setEdges]);
  const onConnect = useCallback((connection) => connect(connection), [connect]);
